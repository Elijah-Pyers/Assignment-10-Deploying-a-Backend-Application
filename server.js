...
require('dotenv').config();              // or: require('dotenv').config({ path: './Assignment_10.env' });
...
// ---- Safety check for JWT secret ----
if (!process.env.JWT_SECRET) {
  console.warn('WARNING: JWT_SECRET is not set! Using an insecure fallback secret (dev only).');
}
...
function requireAuth(req, res, next) {
  ...
  try {
    const decoded = jwt.verify(
      token,
      process.env.JWT_SECRET || 'dev_fallback_secret' // ensure a value
    );
    req.user = decoded;
    next();
  } catch (error) {
